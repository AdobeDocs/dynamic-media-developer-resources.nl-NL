...
Alle visuele aanpassingen en de meeste gedragsaanpassingen worden gedaan door een douane CSS te creëren.
...
Het is ook belangrijk om de volgende CSS-declaratie van de standaard viewer-CSS te behouden zoals deze nodig is voor inline zoomfunctionaliteit:
...
Houd er bij het maken van aangepaste CSS rekening mee dat de viewer `.s7flyoutviewer` -klasse aan het DOM-containerelement. Als u een extern CSS-bestand gebruikt dat wordt doorgegeven met `style=` gebruiken `.s7flyoutviewer` klasse als bovenliggende klasse in afstammende kiezer voor uw CSS-regels. Als u ingesloten stijlen op de webpagina uitvoert, kwalificeert u deze kiezer ook als volgt met een id van het DOM-containerelement:

`#<containerId>.s7flyoutviewer`

## Responsieve CSS maken {#section-c1e74f5114ad418884ca1c95f5ea5b63}

Het is mogelijk om verschillende apparaten en insluitingsgrootten in CSS als doel in te stellen om de weergave van uw inhoud te wijzigen, afhankelijk van het apparaat van de gebruiker of een bepaalde webpaginalay-out. Deze mogelijkheid omvat, maar is niet beperkt tot, verschillende webpaginalay-outs, de grootte van gebruikersinterface-elementen en de resolutie van illustraties.
...
Om responsieve, ontworpen CSS te helpen maken, ondersteunt de viewer CSS-markeringen die speciale CSS-klassen dynamisch aan het containerelement op het hoogste niveau zijn toegewezen. De toewijzingen zijn gebaseerd op de grootte van de uitvoeringsviewer en het invoertype dat op het huidige apparaat wordt gebruikt.
...
Het gebruik van CSS-markeringen is de meest flexibele manier om responsieve, ontworpen CSS samen te stellen. Hiermee kunt u niet alleen de schermgrootte van het apparaat instellen, maar ook de daadwerkelijke viewergrootte. Dit kan handig zijn voor responsieve webpaginalay-outs.
...
Bij een klassieke benadering van opmaak zou de CSS een afzonderlijke verwijzing naar het afzonderlijke afbeeldingsbestand op de server hebben voor elke status van het interface-element. Hier volgt een voorbeeld-CSS voor het opmaken van een schuifknop:
