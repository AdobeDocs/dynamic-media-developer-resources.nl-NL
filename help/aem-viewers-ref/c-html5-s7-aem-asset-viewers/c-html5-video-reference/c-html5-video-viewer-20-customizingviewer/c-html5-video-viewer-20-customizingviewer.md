...
Wanneer u aangepaste CSS maakt, moet u niet vergeten dat de viewer hieraan toewijzingen toekent `.s7videoviewer` -klasse aan het DOM-containerelement. Als u een extern CSS-bestand gebruikt dat samen met de `style=` gebruiken `.s7videoviewer` klasse als bovenliggende klasse in afstammende kiezer voor uw CSS-regels. Als u stijlen op de webpagina insluit, kwalificeert u deze kiezer ook als volgt met een id van het DOM-containerelement:
...
Om u te helpen responsieve, ontworpen CSS te maken, ondersteunt de viewer CSS-markeringen die speciale CSS-klassen dynamisch zijn toegewezen aan het containerelement op het hoogste niveau. De toewijzing is gebaseerd op de grootte van de uitvoeringsviewer en het invoertype dat op het huidige apparaat wordt gebruikt.
...
* Alle paden naar externe elementen in CSS worden omgezet op de CSS-locatie en niet op de locatie van de HTML-pagina van de viewer. Vergeet niet voor deze regel rekening te houden wanneer u de standaard-CSS naar een andere locatie kopieert. Kopieer de standaardelementen of werk paden bij in de aangepaste CSS.
