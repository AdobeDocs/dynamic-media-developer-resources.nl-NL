...
>[!NOTE]
>
>Deze knop ondersteunt de `state` kenmerkkiezer en geselecteerde kenmerkkiezers, die kunnen worden gebruikt om verschillende skins toe te passen op verschillende knoptoestanden. Met name: `selected='true'` komt overeen met de status wanneer bijschriften zichtbaar zijn en `selected='false'` wordt gebruikt wanneer bijschriften worden verborgen.
...
Voorbeeld - Aan opstelling een Gesloten knoop van de Titel die 28 x 28 pixel is, geplaatst vier pixel vanaf de bovenkant, en 68 pixel vanaf de juiste rand van de controlebar. En ten slotte wordt voor elk van de vier verschillende knoptoestanden een andere afbeelding weergegeven wanneer deze is geselecteerd of niet.
