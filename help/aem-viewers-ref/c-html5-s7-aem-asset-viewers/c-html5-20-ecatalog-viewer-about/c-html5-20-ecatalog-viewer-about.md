...
1. De viewer initialiseren.
   ...
   Hieronder ziet u een voorbeeld van het maken van een viewer-instantie, het doorgeven van de minimaal benodigde configuratieopties aan de constructor en het aanroepen van de `init()` methode. In het voorbeeld wordt ervan uitgegaan dat `eCatalogViewer` de viewer-instantie is; de naam van de plaatsaanduiding `s7viewer` is `DIV`; Dit `http://s7d1.scene7.com/is/image/` is de URL voor Beeldbewerking en `Viewers/Pluralist` is het element:
