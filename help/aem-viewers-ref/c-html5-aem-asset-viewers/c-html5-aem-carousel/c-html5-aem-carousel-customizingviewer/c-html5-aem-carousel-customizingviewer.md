...
De viewer wordt geleverd met vier CSS-bestanden die niet in de box staan, voor numerieke en gestippelde sets-indicaties, elk in een &quot;licht&quot; en &quot;donker&quot; kleurenschema. De versie &quot;Gestippeld licht&quot; wordt standaard gebruikt, maar u kunt gemakkelijk naar een andere versie schakelen door verschillende standaard-CSS te gebruiken en de `SetIndicator.mode` parameter. Andere standaard-CSS vindt u op de volgende locatie:
...
Houd er bij het maken van aangepaste CSS rekening mee dat de viewer `.s7carouselviewer` -klasse aan het DOM-containerelement. Als u een extern CSS-bestand gebruikt dat wordt doorgegeven met de `style=` gebruiken `.s7carouselviewer` klasse als bovenliggende klasse in afstammende kiezer voor uw CSS-regels. Als u ingesloten stijlen toevoegt op de webpagina, kwalificeert u deze kiezer ook als volgt met een id van het DOM-containerelement:
