...
Houd er bij het maken van aangepaste CSS rekening mee dat de viewer `.s7interactivevideoviewer` klasse toewijst aan het container DOM-element. Als u extern CSS-bestand gebruikt dat met `style=` de opdracht is doorgegeven, gebruikt u `.s7interactivevideoviewer` klasse als bovenliggende klasse in de onderliggende kiezer voor uw CSS-regels. Als u ingesloten stijlen op de webpagina uitvoert, kwalificeert u deze kiezer als volgt aanvullend met een id van het DOM-containerelement:
...
De derde groep CSS-markeertekens bevat `.s7device_landscape` en `.s7device_portrait`. `.s7device_landscape` wordt ingesteld als het aanraakapparaat liggend is; wordt `.s7device_portrait` gebruikt wanneer het aanraakapparaat wordt gedraaid naar de staande oriëntatie. Deze CSS-markeringen zijn alleen bedoeld voor gebruik op desktopsystemen.
