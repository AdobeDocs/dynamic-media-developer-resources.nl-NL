...
U wordt aangeraden dat viewers op Adobe deze testomgeving testen. Op de volgende website vindt u instructies over hoe u uw systeem kunt instellen op toegang tot de Adobe-staging-server:
...
* Extra ondersteuning voor de Microsoft® Edge-browser.
* De functie Favorieten is toegevoegd aan de eCatalog-viewer.
* Extra ondersteuning voor het uitvoeren van een viewer in de container die is verborgen met `display:none` CSS of wordt losgekoppeld van het DOM.
...
* Viewers: tweede en volgende oproep van `init()` moet worden genegeerd.
