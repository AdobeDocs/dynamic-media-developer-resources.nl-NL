---
description: De onderwerpen in deze sectie beschrijven de syntaxis en de eigenschappen van de protocollen van HTTP.
seo-description: De onderwerpen in deze sectie beschrijven de syntaxis en de eigenschappen van de protocollen van HTTP.
seo-title: Syntaxis en kenmerken
solution: Experience Manager
title: Syntaxis en kenmerken
topic: Scene7 Image Serving - Image Rendering API
uuid: 5f8df6a2-a8f9-4582-9e1e-8f26acf61df4
translation-type: tm+mt
source-git-commit: 94a26628ec619076f0942e9278165cc591f1c150

---


# Syntaxis en kenmerken{#syntax-and-features}

De onderwerpen in deze sectie beschrijven de syntaxis en de eigenschappen van de protocollen van HTTP.

* [Basissyntaxis van HTTP-protocol voor afbeeldingen](r-basic-syntax.md)
* [Overzicht van de opdracht HTTP-protocol voor Image Serving](c-command-overview/c-command-overview.md)
* [Overzicht van lagen](c-layering-overview/c-layering-overview.md)
* [Laagbewerkingen](c-layering-overview/c-layer-operations.md)
* [Laageffecten](r-layer-effects.md)
* [Nesten en insluiten aanvragen](r-request-nesting-and-embedding.md)
* [Vervangende variabelen](r-is-http-substitution-variables.md)
* [Opdrachtmacro&#39;s](r-is-http-command-macros.md)
* [Afbeeldingscatalogi](r-image-catalogs.md)
* [Omzetten van object-id](r-object-id-translation.md)
* [Tekstreeks lokaliseren](r-text-string-localization.md)
* [SVG-ondersteuning](r-svg-support.md)
* [Voorbewerking aanvragen](r-request-preprocessing.md)
* [Afbeeldingen met hyperlinks](r-image-maps.md)
* [Kleurbeheer van afbeeldingsserver](r-color-management.md)
* [HTTP-codering van afbeeldingsserver](r-http-encoding.md)
* [Verzoek vergrendelen](r-request-locking.md)
* [Vragen om verwarring](r-request-obfuscation.md)
* [Watermerken](r-watermarks.md)
* [Statische (niet-afbeeldings) inhoud bedienen](r-serving-static-non-image-content.md)
* [Mediasetaanvragen](r-media-set-requests.md)
