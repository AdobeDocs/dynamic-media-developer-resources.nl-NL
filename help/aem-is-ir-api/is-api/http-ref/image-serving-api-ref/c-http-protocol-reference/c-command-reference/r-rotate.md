...
Positieve hoeken roteren rechtsom. Het laagankerpunt ( `anchor=` of `catalog::Anchor`) fungeert als het middelpunt van de rotatie. De laagrechthoek wordt zo nodig vergroot en rondom de geroteerde gegevens geplaatst om uitsnijden te voorkomen. De rotatie wordt toegepast nadat het achtergrondgebied van de laag met `color=`. `bgColor=` U kunt achtergrondkleur toevoegen aan het selectiekader na het roteren.
...
Wij passen `size=` op laag 0 toe om de meningsgrootte te plaatsen, eerder dan te gebruiken `wid=` en `hei=`. Hierdoor kan `myImageId` de grootte worden aangepast zonder de uiteindelijke grootte van `labelImage`. We moeten ook specificeren `scl=1`, anders kan de samengestelde afbeelding worden geschaald naar `attribute::DefaultPix` (tenzij die is ingesteld op 0,0). `color=` Hiermee voegt u vóór de rotatie de halfdekkende achtergrondkleur toe aan het tekstvak.

De oorsprong van beide lagen wordt ingesteld op de linkerbovenhoek om de gewenste uitlijning te verkrijgen. Het oorsprongpunt voor laag 1 is van toepassing `labelImage`nadat deze is geroteerd.
