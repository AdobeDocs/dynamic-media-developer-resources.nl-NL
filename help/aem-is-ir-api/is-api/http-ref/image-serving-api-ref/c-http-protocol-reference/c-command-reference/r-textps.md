...
Laagkenmerk. Is van toepassing op `layer=0` if `layer=comp`. Deze lagen zijn wederzijds exclusief met `src=` en `text=` in dezelfde laag. De laatste instantie van `text=`, `textPs=`en `src=` prevaleert en bepaalt of dit een afbeelding of een tekstlaag is. Genegeerd door effectlagen.
