...
Als `attribute::DefaultImageMode=0`, *`object`* vervangt het volledige originele verzoek, zelfs als slechts één beeld in een multi-beeldsamenstelling ontbreekt. De enige bevelen die van het originele verzoek worden behouden zijn: `wid=`, `hei=`, `fmt=`, `qlt=`.
...
Request-kenmerk. Is van toepassing ongeacht de huidige laaginstelling. Genegeerd als `req=` een andere dan `img` of `tmb`.
...
Retourneert de functie Afbeeldingsserver `DefaultImageMode=0` wanneer aanvragen voor geneste rendering van afbeeldingen of FXG-rendering mislukken.
