...
De Server van het beeld omvat een versieringsmechanisme dat kan helpen de kans verminderen dat een verouderd geheim voorgeheugeningang door een toepassing wordt gebruikt. Dit mechanisme omvat het gebruik `req=props` om versie-id-tekenreeksen te verkrijgen voor afbeeldingsgegevens en metagegevens (zoals afbeeldingskaart of zoomdoelgegevens). De versie-id-tekenreeks wordt vervolgens toegevoegd aan cacheable Image Serving-aanvragen met de `id=` opdracht.

Wanneer een bronafbeelding of metagegevens worden gewijzigd, verandert ook de bijbehorende versie-id-waarde. Als u een bijgewerkte versie-id-waarde opneemt met de `id=` opdracht, hebt u geen toegang meer tot oude cachemarkeringen.

In de volgende tabel worden de tekenreeks met de versie-id weergegeven die voor elk `req=` type moet worden gebruikt:
...
`req=` typen die niet hierboven zijn vermeld, hebben een korte TTL ( `attribute::NonImgExpiration`) of de reacties zijn helemaal niet in de cache te plaatsen; het heeft geen enkel voordeel dat dergelijke verzoeken `id=` worden opgenomen .
...
Zie de beschrijving van [rect=](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-command-reference/r-rect.md#reference-520b90d30b4c4b4692a723e4df6adaf3) voor bijvoorbeeld gebruik.
