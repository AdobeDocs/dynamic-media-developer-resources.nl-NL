...
Hiermee wordt de groep of het object geselecteerd op de pixelcoördinaten die worden opgegeven door *`x, y`* en wordt een nieuw MSS gestart. Als er geen selecteerbaar object is op de keuzesocatie of als de keuzesocatie niet geldig is, `attribute::OnFailSel` wordt de actie uitgevoerd die is opgegeven door.

*`level`* Hiermee geeft u aan of u de buitenste groep wilt selecteren of een diepgaande lijn naar een geneste groep of object wilt doorlopen. Als *`level`* niet is opgegeven, wordt de buitenste groep geselecteerd. Stel de waarde in op 1 om één groepsniveau onder de buitenste groep te selecteren. Stel dit in op een groot getal (bijvoorbeeld 99) om het binnenste selecteerbare object of de binnenste groep te selecteren.
...
*`x, y`* moet liggen in het bereik 0, 0 (linkerbovenhoek van de afbeelding) tot *`wid`*-1, *`hei`*-1 (rechteronderhoek van de afbeelding), waarbij *`wid`* en *`hei`* de grootte van de niet-geschaalde vignetweergave is.

Indien opgegeven, *`level`* moet deze 0 of groter zijn.
