...
Het inkleuringsalgoritme voor het renderen van afbeeldingen is heel eenvoudig: de componentwaarden van `bgc=` worden afgetrokken van die van de structuurpixels, `color=` wordt toegevoegd en ten slotte wordt het resultaat geknipt naar `0,0,0` en `255,255,255`.

Voor het typische gebruik van textuurkleuring, `bgc=` zou de waarde voor de belangrijkste of dominante kleur in het textuurbeeld kunnen zijn. Scene7 de Authoring van het Beeld verstrekt semi-automatische hulpmiddelen die redelijke `bgc=` kleurenwaarden uit textuurbeelden halen.

Wanneer een structuurmateriaal wordt toegepast op een niet-tekentabel vignetobject, `bgc=` wordt dit toegepast als de voorgrondkleur als deze niet `color=` is opgegeven.
