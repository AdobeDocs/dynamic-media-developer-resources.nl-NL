...
* Het aantal lijnen zal niet het maximum van overschrijden `\copyfitmaxlines` het instellen en het aantal expliciete regels in de tekstinvoer.
...
* De `id=` is vereist wanneer u de opdracht `rect=` gebruiken met een afbeelding of maskerverzoek.
...
* Wanneer u een bedrijf gebruikt dat in de URL is gedefinieerd en een ander bedrijf dat voor de `src=` of `mask=` voorvoegsel, moet u een voorwaartse schuine streep aan het bedrijf voor bepalen `src=` of `mask=` voor deze vorm van verzoek om te werken.
...
`ImageConvert`soms vastloopt met een segmentatiefout wanneer deze wordt gestopt met een `control-c`.
